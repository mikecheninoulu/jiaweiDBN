...
# just to accommodate some frame didn't start right from the begining
    start_label = numpy.concatenate((range(0,210,10), range(1,210,10), range(2,210,10), range(3,210,10) )) #jw
    end_label   = numpy.concatenate((range(9,210,10), range(8,210,10), range(7,210,10), range(6,210,10) )) #jw
    begin_frame = []
    end_frame = []
    pred_label = []
    ...
    while(frame< path.shape[-1]-1):
        if path[frame-1]==210 and path[frame] in start_label: #jw
            begin_frame.append(frame)
            # python integer divsion will do the floor for us :)
            pred_label.append(path[frame]/10 + 1)
            frame += 1
        elif path[frame] in end_label and path[frame+1]==210: #jw
...
#     bg_frame = 0
#     while(bg_frame < begin_frame.shape[-1]-1):
#         print("%%%%%%%%%%")
#         print(bg_frame)
#         print(begin_frame[bg_frame])
#         print(end_frame[bg_frame])
#         print("^sec")
#             
#         frames = end_frame[bg_frame] - begin_frame[bg_frame]
#         ibf = begin_frame[bg_frame]
#         ief = end_frame[bg_frame]
#             
#         for t in range(0, frames):
#             print(ibf+t)
#             if path[ibf+t+1] == path[ibf+t]:
#                 begin_frame[bg_frame] += 1
#             else:
#                 break
#         bg_frame += 1
